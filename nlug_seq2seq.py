...
class AttnDecoderRNN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, label_size, output_size, word_vectors, drop=0.1, max_length=MAX_LEN):
...
class NLUG_generator():
    ...
    def train_step(self, input_tensor, nlg_class, target_tensor, criterion, max_length=MAX_LEN):
        ...
        with torch.no_grad():
            ...
            for di in range(max_length):
                ...
                if decoder_input.item() == EOS_token:
                    ...
                    decoded_words.append(self.idx2word[idx.item()])
            
            return decoded_words, decoder_attentions[:di+1]

    def load_model(self, model_path="nlug_batch.pt"):
    ...
    def train(self, lr=1e-4, epochs=100):
        ...
        data_x_seq = [self.sent2tensor(p[0]) for p in self.dataset]
        data_x_class = [self.label2vec(self.label2list(p[1])) for p in self.dataset]
        ...
        for e in range(epochs):
            ...
            for step in range(math.floor(len(train_seq)/BATCH_SIZE)):
                ...
                loss = 0
                for s in next_batch:
                    loss += self.train_step(data_x_seq[s], data_x_class[s], data_y[s], criterion)
            ...
            torch.save(self.encoder.state_dict(), 'nlug_batch.pt')           
            #validate
            val_bleu_score = 0.0
            for vs in val_seq:
